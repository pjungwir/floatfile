...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < 100; i++) {
    x_fd       = open("/var/lib/postgresql/9.6/main/floatfile/19949/ds3820ch1.v", O_RDONLY);
    if (x_fd == -1) { perror("x_fd"); exit(1); }
    x_nulls_fd = open("/var/lib/postgresql/9.6/main/floatfile/19949/ds3820ch1.n", O_RDONLY);
    if (x_nulls_fd == -1) { perror("x_nulls_fd"); exit(1); }
    y_fd       = open("/var/lib/postgresql/9.6/main/floatfile/19949/ds3820ch2.v", O_RDONLY);
    if (y_fd == -1) { perror("y_fd"); exit(1); }
    y_nulls_fd = open("/var/lib/postgresql/9.6/main/floatfile/19949/ds3820ch2.n", O_RDONLY);
