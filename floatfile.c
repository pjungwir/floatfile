...
Datum floatfile_with_bounds_to_hist(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(floatfile_with_bounds_to_hist);
/**
 * floatfile_to_hist - Uses a floatfile to build a histogram.
 */
Datum
floatfile_with_bounds_to_hist(PG_FUNCTION_ARGS)
{
  char *xs_filename;
  int32 xs_filename_hash;
  char *ts_filename;
  int32 ts_filename_hash;
  int x_fd = 0, x_nulls_fd = 0;
  int t_fd = 0, t_nulls_fd = 0;
  float8 x_min, x_width;
  int32 x_count;
  float8 t_min, t_max;
  ssize_t min_pos, max_pos;
  // Make sure `counts` has the same width as Datum
  // so we can avoid a memcpy:
#ifdef SAFE_TO_CAST_FLOATS_AND_DATUMS
  int64 *counts = NULL;
  ...
  ts_filename = GET_STR(PG_GETARG_TEXT_P(4));
  xs_filename = GET_STR(PG_GETARG_TEXT_P(0));

  t_min = PG_GETARG_FLOAT8(5);
  t_max = PG_GETARG_FLOAT8(6);

  x_min = PG_GETARG_FLOAT8(1);
  x_width = PG_GETARG_FLOAT8(2);
  x_count = PG_GETARG_INT32(3);

  ts_filename_hash = hash_filename(ts_filename);
  xs_filename_hash = hash_filename(xs_filename);
  DirectFunctionCall2(pg_advisory_lock_shared_int4, FLOATFILE_LOCK_PREFIX, ts_filename_hash);
  DirectFunctionCall2(pg_advisory_lock_shared_int4, FLOATFILE_LOCK_PREFIX, xs_filename_hash);

  if (open_floatfile_for_reading(NULL, ts_filename, &t_fd, &t_nulls_fd) == -1) {
    errstr = strerror(errno);
    goto bail;
  }
  ...
  find_bounds_start_end(t_fd, t_nulls_fd, t_min, t_max, &min_pos, &max_pos, &errstr);
  if (errstr) goto bail;
  if (min_pos == -1 || max_pos == -1) {
    // The histogram is empty so just return, but with no error.
    goto bail;
  }

  build_histogram_with_bounds(x_fd, x_nulls_fd, x_min, x_width, x_count,
                  counts, min_pos, max_pos, &errstr);

bail:
  if (x_fd       && close(x_fd))       errstr = "Can't close x_fd";
  if (x_nulls_fd && close(x_nulls_fd)) errstr = "Can't close x_nulls_fd";
  DirectFunctionCall2(pg_advisory_unlock_shared_int4, FLOATFILE_LOCK_PREFIX, xs_filename_hash);
  if (t_fd       && close(t_fd))       errstr = "Can't close t_fd";
  if (t_nulls_fd && close(t_nulls_fd)) errstr = "Can't close t_nulls_fd";
  DirectFunctionCall2(pg_advisory_unlock_shared_int4, FLOATFILE_LOCK_PREFIX, ts_filename_hash);
  if (errstr) elog(ERROR, "%s", errstr);
...
Datum floatfile_in_tablespace_with_bounds_to_hist(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(floatfile_in_tablespace_with_bounds_to_hist);
/**
 * floatfile_in_tablespace_with_bounds_to_hist - Uses a floatfile to build a histogram.
 */
Datum
floatfile_in_tablespace_with_bounds_to_hist(PG_FUNCTION_ARGS)
{
  // TODO: float4 instead of float8??
  char *xs_tablespace = NULL;
  char *xs_filename;
  int32 xs_filename_hash;
  char *ts_tablespace = NULL;
  char *ts_filename;
  int32 ts_filename_hash;
  int x_fd = 0, x_nulls_fd = 0;
  int t_fd = 0, t_nulls_fd = 0;
  ...
  if (PG_ARGISNULL(1) ||
      PG_ARGISNULL(2) || PG_ARGISNULL(3) || PG_ARGISNULL(4) ||
      PG_ARGISNULL(6) ||
      PG_ARGISNULL(7) || PG_ARGISNULL(8)) {
    PG_RETURN_NULL();
  }

  if (!PG_ARGISNULL(5)) ts_tablespace = GET_STR(PG_GETARG_TEXT_P(5));
  ts_filename = GET_STR(PG_GETARG_TEXT_P(6));

  if (!PG_ARGISNULL(0)) xs_tablespace = GET_STR(PG_GETARG_TEXT_P(0));
  xs_filename = GET_STR(PG_GETARG_TEXT_P(1));

  t_min = PG_GETARG_FLOAT8(7);
  t_max = PG_GETARG_FLOAT8(8);
  ...
  if (open_floatfile_for_reading(ts_tablespace, ts_filename, &t_fd, &t_nulls_fd) == -1) {
    errstr = strerror(errno);
    goto bail;
  }
...
Datum floatfile_in_tablespace_to_hist2d(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(floatfile_in_tablespace_to_hist2d);
/**
 * floatfile_in_tablespace_to_hist2d - Uses two floatfiles to build a 2d histogram.
 */
Datum
floatfile_in_tablespace_to_hist2d(PG_FUNCTION_ARGS)
{
  ...
  y_min = PG_GETARG_FLOAT8(5);
  x_width = PG_GETARG_FLOAT8(6);
  y_width = PG_GETARG_FLOAT8(7);
  x_count = PG_GETARG_INT32(8);
  y_count = PG_GETARG_INT32(9);
  ...
  if (open_floatfile_for_reading(ys_tablespace, ys_filename, &y_fd, &y_nulls_fd) == -1) {
    errstr = strerror(errno);
    goto bail;
  }
...
Datum floatfile_with_bounds_to_hist2d(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(floatfile_with_bounds_to_hist2d);
/**
 * floatfile_with_bounds_to_hist2d - Uses two floatfiles to build a 2d histogram.
 */
Datum
floatfile_with_bounds_to_hist2d(PG_FUNCTION_ARGS)
{
  // TODO: float4 instead of float8??
  char *xs_filename;
  char *ys_filename;
  ...
  if (PG_ARGISNULL(0) || PG_ARGISNULL(1) || PG_ARGISNULL(2) || PG_ARGISNULL(3) ||
      PG_ARGISNULL(4) || PG_ARGISNULL(5) || PG_ARGISNULL(6) || PG_ARGISNULL(7) ||
      PG_ARGISNULL(8) || PG_ARGISNULL(9) || PG_ARGISNULL(10)) {
    PG_RETURN_NULL();
  }

  ts_filename = GET_STR(PG_GETARG_TEXT_P(8));
  xs_filename = GET_STR(PG_GETARG_TEXT_P(0));
  ys_filename = GET_STR(PG_GETARG_TEXT_P(1));

  t_min = PG_GETARG_FLOAT8(9);
  t_max = PG_GETARG_FLOAT8(10);
  ...
  build_histogram_2d_with_bounds(x_fd, x_nulls_fd, x_min, x_width, x_count,
                     y_fd, y_nulls_fd, y_min, y_width, y_count,
                     counts, min_pos, max_pos, &errstr);
...
Datum floatfile_in_tablespace_with_bounds_to_hist2d(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(floatfile_in_tablespace_with_bounds_to_hist2d);
/**
 * floatfile_in_tablespace_with_bounds_to_hist2d - Uses two floatfiles to build a 2d histogram.
 */
Datum
floatfile_in_tablespace_with_bounds_to_hist2d(PG_FUNCTION_ARGS)
{
  // TODO: float4 instead of float8??
  char *xs_tablespace = NULL;
  char *xs_filename;
  ...
  if (PG_ARGISNULL(1) ||
      ...
      PG_ARGISNULL(7) || PG_ARGISNULL(8) || PG_ARGISNULL(9) ||
      PG_ARGISNULL(11) ||
      PG_ARGISNULL(12) || PG_ARGISNULL(13)) {
    PG_RETURN_NULL();
  }

  if (!PG_ARGISNULL(10)) ts_tablespace = GET_STR(PG_GETARG_TEXT_P(10));
  ts_filename = GET_STR(PG_GETARG_TEXT_P(11));
  if (!PG_ARGISNULL(0)) xs_tablespace = GET_STR(PG_GETARG_TEXT_P(0));
  xs_filename = GET_STR(PG_GETARG_TEXT_P(1));
  if (!PG_ARGISNULL(2)) ys_tablespace = GET_STR(PG_GETARG_TEXT_P(2));
  ys_filename = GET_STR(PG_GETARG_TEXT_P(3));

  t_min = PG_GETARG_FLOAT8(12);
  t_max = PG_GETARG_FLOAT8(13);
